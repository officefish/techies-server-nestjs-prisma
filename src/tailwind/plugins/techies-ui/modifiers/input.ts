const setupInput = (colorTag: string, outlineVariable: string) => {
  return {
    [`&-${colorTag}`]: {
      [`@apply border-${colorTag} dark:border-${colorTag}-dark`]: {},
      '&:focus': {
        outline: `2px solid hsl(var(${outlineVariable}))`,

        ':is(.dark) &': {
          outline: `2px solid hsl(var(${outlineVariable}-d))`,
        },
      },
    },
  }
...
export const input = {
  '.input': {
    ...
    '&-ghost': {
      '@apply bg-base-100/5 dark:bg-base-content-dark/5': {},
      '&:focus': {
        '@apply bg-base-100 dark:bg-base-100-dark': {},
        '@apply text-base-content dark:text-base-content-dark': {},
        'box-shadow': 'none',
    ...
    ...setupInput('primary', '--p'),
    ...setupInput('secondary', '--s'),
    ...setupInput('accent', '--a'),
    ...setupInput('info', '--in'),
    ...setupInput('success', '--su'),
    ...setupInput('warning', '--wa'),
    ...setupInput('error', '--er'),
