const setupCornerBg = (dark: boolean) => {
    ...
    transparent var(--tab-grad), var(--tab-border-color${dark ? '-dark' : ''}) calc(var(--tab-grad) + 0.3px), 
    var(--tab-border-color${dark ? '-dark' : ''}) calc(var(--tab-grad) + var(--tab-border, 1px)), 
    var(--tab-bg${dark ? '-dark' : ''}) calc(var(--tab-grad) + var(--tab-border, 1px) + 0.3px))`
